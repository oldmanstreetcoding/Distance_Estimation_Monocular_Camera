...
def inference_on_dataset(cfg, model, data_loader, iteration):
    ...
    with inference_context(model), torch.no_grad():
        for idx, dataset_dict in enumerate(data_loader):
            ...
            text_to_write = ''
            dist_f = open(os.path.join(cfg.OUTPUT_DIR, 'evaluation', iteration, 'dist.txt'), 'a')
            distance_to_write = file_name.split('/')[-1][:-4] + '.png'
            ...
            for boxind in range(pred_boxes.shape[0]):
                ...
                if cfg.TEST.VISUALIZE and iteration == "test":
                    ...
                    close_x, close_z = draw_bev(image_bev, x3d, z3d, l3d, w3d, ry3d)
                    draw_image(image_2d, point_2d,close_x,close_z)
                    
                    distance = math.sqrt(close_x*close_x+ close_z*close_z)
                    distance_to_write += (', {:.2f}'.format(distance))
                    

            file.write(text_to_write)
            file.close()
            dist_f.write(distance_to_write + '\n')
            dist_f.close()
